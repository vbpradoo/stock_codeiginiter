...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			// console.log(tmpcm.width);
			if(tmpcm.name === 'subgrid'){
				tmpcm.colmenu = true;
				tmpcm.fixed = false;
				tmpcm.width = 150;
			}
...
addSubGridCell :function (pos,iRow) {
	...
	return "<td role=\"gridcell\" aria-describedby=\""+sid+"_subgrid\" class=\"ui-sgcollapsed sgcollapsed mycollapsed\" "+prp+"><a style='cursor:pointer;' class='ui-sghref'><span class='" + icb.icon_base +" "+ic+"'></span></a></td>";
},
...
$.jgrid.extend({
	...
	exportToExcel : function ( o ) {
		...
		this.each(function() {
			...
			// console.log($t.addLocalData(true));
			// console.log(data);
			for ( j=0, ien=cm.length ; j<ien ; j++ ) {
				...
				//CUSTOME CASE
				if(cm[j].name === "Vendido" || cm[j].name ==="Buttons" )
                    continue;
